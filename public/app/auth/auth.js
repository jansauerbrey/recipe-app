...
// Auth
      ...
      updateUserSettings = function(callback) {
        if (data.currentUser !== undefined) {
          ...
          User.update({
            ...
            fullname: data.currentUserCopy.fullname,
            settings: data.currentUserCopy.settings
          }, function(response) {
            for (const attrname in response) {
  ...
  .factory('AuthenticationService', ['$http', '$localStorage', '$state', '$stateParams', '$rootScope', 'UserService', 'BASE_URI', 'AlertService',
    function($http, $localStorage, $state, $stateParams, $rootScope, UserService, BASE_URI, AlertService) {
      const logIn = function(username, password, autologin) {
          if (username !== undefined && password !== undefined) {
            $http.post(BASE_URI + 'api/user/login', {
              username: username,
              password: password,
              autologin: autologin
        ...
        requestNewPassword = function(username) {
          if (username !== undefined) {
            return $http.post(BASE_URI + 'api/user/forgot', {
              username: username
            }).success(function(data) {
        ...
        resetPassword = function(password, passwordConfirmation) {
          if (password !== undefined && password === passwordConfirmation) {
            return $http.put(BASE_URI + 'api/user/reset/' + $stateParams.token, {
              password: password,
