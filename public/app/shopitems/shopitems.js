...
//---------------
  ...
  .factory('ShopitemService', ['$rootScope', '$timeout', '$q', 'UserService', 'Shopitems', function($rootScope, $timeout, $q, UserService, Shopitems){
    			...
    			const data = {shopitems: [], alerts: [], autoupdate: true, pauseAutoupdate: 0};
      ...
      Shopitems.query(function(response) {
        ...
        for(i=0;i<response.length;i++){
          ...
          if (response[i].ingredient && response[i].unit && response[i].amount) { // check if item is complete
            ...
            const obj = {ingredient:response[i].ingredient, unit:response[i].unit, order: order, completed: response[i].completed, details: [], amount: 0};
    ...
    const addShopitem = function(newshopitem){
      ...
      const newshopitemClean = new Shopitems({ingredient: newshopitem.ingredient, unit: newshopitem.unit, amount: newshopitem.amount, completed: false , expire_date: expDate });
...
// Shopitems
  ...
  .controller('ModalShopitemAddController', ['$scope', '$stateParams', '$uibModalInstance', 'TAIngredients', 'units', 'ShopitemService', function ($scope, $stateParams, $uibModalInstance, TAIngredients, units, ShopitemService) {
    ...
    $scope.ok = function(validForm){
      ...
      const item = {amount: $scope.newamount, unit: $scope.newunitobject, ingredient: $scope.newingredient};
      	ShopitemService.addShopitem(item);
      $uibModalInstance.close();
    };
