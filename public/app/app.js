      ...
      link: function(scope, element, attrs) {
        element.bind('click', function() {
          ...
          if (scope.$parent.schedule && scope.$parent.schedule.recipe && scope.$parent.schedule.recipe.name) {
            confirmMessage = 'Are you sure to delete the recipe ' + scope.$parent.schedule.recipe.name + ' from schedule?';
          } else if (scope.$parent.recipe && scope.$parent.recipe.name) {
            confirmMessage = 'Are you sure to delete the recipe ' + scope.$parent.recipe.name + '?';
          } else {
...
//---------------
  ...
  .filter('timeFilter', function() {
    ...
    return function(value, unit) {
      const totalSeconds = conversions[unit || 'ss'](value),
        hh = Math.floor(totalSeconds / 3600),
        mm = Math.floor((totalSeconds % 3600) / 60);

      const displayHours = (hh > 0) ? hh : '';
      const displayFormat = (displayHours === '') ? 'mm min' : 'hhh mmm';
      return displayFormat.replace(/hh/, displayHours).replace(/mm/, mm);
    };
  })
