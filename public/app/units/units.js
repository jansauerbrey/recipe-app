...
// Units
  ...
  .controller('UnitDetailCtrl', ['$scope', '$stateParams', 'unit', 'Units', '$state', function ($scope, $stateParams, unit, Units, $state) {
    ...
    $scope.save = function(){
      ...
      const unit_new = new Units({ name: { en: $scope.newunit.name.en, de: $scope.newunit.name.de,  fi: $scope.newunit.name.fi} });
      ...
      .state('admin.units.list', {
        ...
        resolve: {
          unit: ['Units', '$stateParams', function(Units, $stateParams){
            const unit = Units.get({'id': $stateParams.id}, function(response) {
              return response;
            }).$promise;
            return unit;
          }]
