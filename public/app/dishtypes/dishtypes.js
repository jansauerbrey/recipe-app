...
// Dishtypes
  ...
  .controller('DishTypeDetailCtrl', ['$scope', '$stateParams', 'dishtype', 'DishTypes', '$state', function ($scope, $stateParams, dishtype, DishTypes, $state) {
    ...
    $scope.save = function(){
      ...
      const dishtype_new = new DishTypes({ name: { en: $scope.newdishtype.name.en, de: $scope.newdishtype.name.de,  fi: $scope.newdishtype.name.fi} , order: $scope.newdishtype.order, imagePath: $scope.newdishtype.imagePath});
      ...
      .state('admin.dishtypes.edit', {
        ...
        resolve: {
          dishtype: ['DishTypes', '$stateParams', function(DishTypes, $stateParams){
            const dishtype = DishTypes.get({'id': $stateParams.id}, function(response) {
              return response;
            }).$promise;
            return dishtype;
          }]
